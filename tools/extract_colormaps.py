...
# The entry tables are one compact line per colormap. rustfmt wants to explode
# each into six, turning ~125 readable rows into ~750 lines of noise - so the
# generated statics opt out. This must be emitted here rather than fixed up
# afterwards: `cargo fmt --all` is a CI gate, and without it every re-run of
# this script would leave the workspace unformatted.
...
def main() -> None:
    ...
    with REGISTRY_RS.open("w") as f:
        ...
        f.write(RUSTFMT_SKIP)
        f.write("pub static CONTINUOUS: &[ContinuousEntry] = &[\n")
        for n in names:
