...
use figurs_core::kurbo::{BezPath, PathEl, Point};
...
impl<'a> Engine<'a> {
    ...
    fn glyph_box(&self, ch: char, size: f32, math: bool) -> Layout {
        ...
        let (adv, ink, italic) = if math {
            ...
            let run = figurs_text::shape_text(font, &ch.to_string(), size);
            let w = figurs_text::run_width(&run);
            let vm = figurs_text::font_vmetrics(font, size);
            return Layout {
                width: w,
                ascent: vm.ascent,
                depth: vm.descent,
    ...
    fn text_run(&self, s: &str, size: f32) -> Layout {
        let run = figurs_text::shape_text(self.body_font, s, size);
        let w = figurs_text::run_width(&run);
        let vm = figurs_text::font_vmetrics(self.body_font, size);
        Layout {
            width: w,
            ascent: vm.ascent,
            depth: vm.descent,
...
pub(crate) fn shaped_text_layout(font: &FontData, s: &str, size: f32) -> Layout {
    let run = figurs_text::shape_text(font, s, size);
    let w = figurs_text::run_width(&run);
    let vm = figurs_text::font_vmetrics(font, size);
    Layout {
        width: w,
