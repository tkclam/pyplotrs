//! figurs-math: a faithful (LaTeX/MathJax-grade) math typesetter.
//!
...
//! Math) rather than hand-tuned constants. Output is ordinary figurs IR — real,
...
//! the positioned [`figurs_core::Node`]s plus those metrics. Plain strings with
...
mod tests {
    ...
    fn radical_rule_connects_to_surd() {
        ...
        for expr in ["$\\sqrt{x}$", "$\\sqrt{\\frac{a+b}{2}}$"] {
            ...
            let mut boxes: Vec<figurs_core::kurbo::Rect> = Vec::new();
            for d in &l.draws {
