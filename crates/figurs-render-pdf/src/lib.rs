//! figurs-render-pdf: renders a [`figurs_core::Scene`] to a PDF document
//! using `krilla`.
//!
//! Text is drawn via `Surface::draw_glyphs` with the pre-shaped glyph
//! IDs/positions from `figurs-text`, so it is embedded as real,
...
fn to_krilla_path(geometry: &figurs_core::kurbo::BezPath) -> Option<Path> {
...
/// Map a [`figurs_core`] alpha (0-255) to a krilla [`NormalizedF32`].
...
impl PdfRenderer {
    fn font_for(&mut self, run_font: &figurs_core::FontData) -> Font {
        self.fonts
            .entry(run_font.key())
            .or_insert_with(|| {
