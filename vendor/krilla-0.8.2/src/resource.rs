...
impl ResourceDictionaryBuilder {
    ...
    pub(crate) fn register_resource<T>(&mut self, obj: T) -> Arc<str>
    where
        T: Resource,
    {
...
pub(crate) struct ResourceMapper<T: ?Sized> {
    forward: Vec<Ref>,
    backward: FxHashMap<Ref, ResourceNumber>,
    // Formatted resource names ("G0", "G1", ...), one per entry in `forward`
    // and filled in lockstep with it. A single reusable resource (e.g. one
    // Form XObject placed at every point of a large scatter) hits
    // `remap_with_name` once per placement, so caching the formatted name
    // here turns every repeat placement into an `Arc` clone instead of a
    // fresh `format!` + allocation.
    names: Vec<Arc<str>>,
    ...
    T: Resource,
{
    ...
    pub(crate) fn remap_with_name(&mut self, ref_: Ref) -> Arc<str> {
        let num = self.remap(ref_);
        match self.names.get(num as usize) {
            Some(name) => name.clone(),
            // `remap` only ever hands out numbers in order (0, 1, 2, ...) as
            // new refs are first seen, and this is the only caller, so
            // `names` is always exactly as long as `forward` was before this
            // call - i.e. `num == names.len()` on every miss.
            None => {
                let name: Arc<str> = Self::name_from_number(num).into();
