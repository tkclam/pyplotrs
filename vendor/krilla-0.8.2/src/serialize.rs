...
pub(crate) struct SerializeContext {
    ...
    ///
    /// Looked up on every draw of a reused resource (e.g. every point of a large
    /// marker-instanced scatter), so this uses the same fast, non-cryptographic
    /// hasher as krilla's other hot lookup caches (see `text/cid.rs`,
    /// `text/mod.rs`) instead of the default SipHash - there's no adversarial
    /// input here to defend against, just a document being built once.
    cached_mappings: FxHashMap<u128, Ref>,
...
impl SerializeContext {
    pub(crate) fn new(mut serialize_settings: SerializeSettings) -> Self {
        ...
        Self {
            cached_mappings: FxHashMap::default(),
            pdf2_ns,
